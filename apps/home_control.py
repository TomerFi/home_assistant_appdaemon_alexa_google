...
hava_profile = {
    ...
    "find_script": "script.icloud_find_hava_iphone",
    "names": [
        "hava",
        "my wife"
    ]
...
tomer_profile = {
    ...
    "find_script": "script.icloud_find_tomer_iphone",
    "names": [
        "tomer",
        "my husband"
    ]
...
network_profile = {
    ...
    "noip_cert": "sensor.ha_noip_cert_expiry"
}
...
def handleNetworkSsml(api, is_new=False):
    ...
    noip_cert = api.get_state(network_profile["noip_cert"])
...
def authorizeAlexaUse(api, application_id, device_id, user_id):
    ...
    if not (debug):
        applications = [] # Add your skill id or leave empty for no skill restriction, multiple skill id's supported.

        devices = [] # Add your device id or leave empty for no device restriction, multiple device id's supported.

        users = [] # Add your user id or leave empty for no user restriction, multiple user id's supported.

        if (not (applications == []) and application_id not in applications):
            api.log("unauthorized application id: {}.".format(application_id))
            authorized = False

        if (not (devices == [])and device_id  not in devices):
            api.log("unauthorized device id: {}.".format(device_id))
            authorized = False

        if (not (users == []) and user_id not in users):
